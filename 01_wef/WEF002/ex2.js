...
let answer = ""
    //Question #2 Remove need to use filter function
...
const numberOfSpade = cards.reduce((count /*2*/ , card /* ['Club', '2']*/ ) => {
    if (card[0] === "Spade") {
        return count + 1; //2
    } else {
        return count; //2
...
console.log(`Count the number of card which is of suit Spade : ${numberOfSpade}`);


//Remove all the card that is smaller than ["Club", "3"].
//=> Keep all of the card >= ["Club", "3"]
const largerThanClub3 = cards.filter(card => compareCard(card, ["Club", "3"]) >= 0);
...
console.log(`Count the number of card which is of suit Diamond or Heart and with the rank larger than or equal to J : ${countDiaHeartLargerJ}`)
...
//     return card;
// });
const club2Diamond = cards.map(card => card[0] === "Club" ? ["Diamond", card[1]] : card);

console.log("Replace all of the cards with suit Club to suit Diamond, keeping the same rank.")
